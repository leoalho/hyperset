...
function moveOther(diffx, diffy, otherColor, index) {
  area.drawBoard();
  area.drawPlayer(
    area.cards[index].x + area.size + diffx,
    area.cards[index].y + diffy,
    otherColor,
  );
  area.drawYou();
  counter--;
...
function drawPoints() {
  var playerPoints =
    ...
    "<br> In total: " +
    player.pointsTotal +
    "</p><p><u>Top 5 players<br> This round: </u><br>";
  ...
  for (let i = 0; i < highscoresToday.length; i++) {
    playerPoints +=
      highscoresToday[i].username + ": " + highscoresToday[i].score + ", <br>";
  }
  playerPoints += "<u>All time:</u><br>";
  for (let i = 0; i < highscoresAllTime.length; i++) {
    playerPoints +=
      highscoresAllTime[i].username +
      ": " +
      highscoresAllTime[i].score +
      ", <br>";
  }
  playerPoints += "</p>";
...
socket.on("highscoresToday", (newScoresToday) => {
  ...
  for (var i = 0; i < highscoresToday.length; i++) {
    highscoreText +=
      ...
      1 +
      ": " +
      highscoresToday[i].username +
      ": " +
      highscoresToday[i].score +
      "<br>";
  }
  $("highscoreText").innerHTML = highscoreText;
});
socket.on("highscoresThisMonth", (newScoresToday) => {
  ...
  for (var i = 0; i < highscoresThisMonth.length; i++) {
    highscoreText +=
      ...
      1 +
      ": " +
      highscoresThisMonth[i].username +
      ": " +
      highscoresThisMonth[i].score +
      "<br>";
  }
  $("highscoreText").innerHTML = highscoreText;
});
socket.on("highscoresThisYear", (newScoresToday) => {
  ...
  for (var i = 0; i < highscoresThisYear.length; i++) {
    highscoreText +=
      ...
      1 +
      ": " +
      highscoresThisYear[i].username +
      ": " +
      highscoresThisYear[i].score +
      "<br>";
  }
  $("highscoreText").innerHTML = highscoreText;
});
